/* Shared MEC registration logic backed by Supabase */
(function () {
  function normalizePhone(input) {
    return (input || "").replace(/\D/g, "").slice(0, 15);
  }

  function initMecRegistration(config) {
    const {
      supabaseUrl,
      supabaseKey,
      ministryKey,
      slots,
      tableSlots = "mec_slots",
      tableRegistrations = "mec_registrations",
    } = config || {};

    const slotSelect = document.getElementById("slot");
    const cuposGrid = document.getElementById("cupos-grid");
    const form = document.getElementById("mec-registro-form");
  ...
  window.initMecRegistration = initMecRegistration;
})();
